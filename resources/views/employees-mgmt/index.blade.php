  ...
  <div class="box-body">
      ...
      <form method="POST" action="{{ route('employee-management.search') }}">
         ...
         @component('layouts.search', ['title' => 'Search'])
          @component('layouts.two-cols-search-row', ['items' => ['Given Name', 'Surname'], 
          'oldVals' => [isset($searchingVals) ? $searchingVals['given_name'] : '', isset($searchingVals) ? $searchingVals['surname'] : '']])
          @endcomponent
        @endcomponent
      </form>
          ...
          <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
            <thead>
              <tr role="row">
                ...
                <th width="8%" class="sorting hidden-xs" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="HiredDate: activate to sort column ascending">Phone Number</th>
                <th width="8%" class="sorting hidden-xs" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Department: activate to sort column ascending">District</th>
                <th width="8%" class="sorting hidden-xs" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Division: activate to sort column ascending">Father's Name</th>
            ...
            @foreach ($employees as $employee)
                <tr role="row" class="odd">
                  ...
                  <td class="sorting_1">{{ $employee->given_name }} {{$employee->surname}}</td>
                  ...
                  <td class="hidden-xs">{{ $employee->father_name }}</td>
